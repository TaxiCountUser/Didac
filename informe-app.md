...
| `reports.js` | Generación de Excel (`exceljs`) y PDF (`pdfmake`). **Excel**: filas ordenadas por cliente (`orderedDetail`) — ingresos agrupados por cliente (Particular primero, resto alfabético), **fila en blanco entre grupos**, ordenados por fecha/hora; los **gastos al final**, por fecha. Importes **numéricos con formato € (EUR)** (`MONEY_FMT`; multi-moneda = futuro). Resumen en negrita con los **gastos encima** de los ingresos: gastos como **categoría · método de pago en la misma línea** → TOTAL Gastos; ingresos por método → TOTAL Ingresos; BALANCE. Un gasto **"Otros"** con texto libre sale en el **listado** como `otros (ITV)` (`catDisplay`) y en el **total/resumen** como `ITV` (`expenseCatKey`). El **PDF** replica el Excel: mismo orden por cliente (`orderedDetail` con filas en blanco) y mismo resumen (`summaryBlock` = gastos categoría·método encima de ingresos). |
