...
| `reports.js` | Generación de Excel (`exceljs`) y PDF (`pdfmake`). **Excel**: filas ordenadas por cliente (`orderedDetail`) — ingresos agrupados por cliente (Particular primero, resto alfabético), **fila en blanco entre grupos**, ordenados por fecha/hora; los **gastos al final**, por fecha. Importes **numéricos con formato € (EUR)** (`MONEY_FMT`; multi-moneda = futuro). Resumen en negrita con los **gastos encima** de los ingresos: gastos como **categoría · método de pago en la misma línea** → TOTAL Gastos; ingresos por método → TOTAL Ingresos; BALANCE. Un gasto **"Otros"** con texto libre sale en el **listado** como `otros (ITV)` (`catDisplay`) y en el **total/resumen** como `ITV` (`expenseCatKey`). El **PDF** replica el Excel: mismo orden por cliente (`orderedDetail` con filas en blanco) y mismo resumen (`summaryBlock` = gastos categoría·método encima de ingresos). Los **importes de los gastos** (solo la cifra €) van en **rojo** (`RED_ARGB`/`RED_HEX`), tanto en el listado como en el resumen. |
...
| **Logs de seguridad (capa B)** | `logSecurityEvent(request, type, opts)` → tabla `security_events` (mig. 077): escalada de privilegios (403 en `adminGuard`), rate-limit 429, token inválido (`getCaller`), login por usuario fallido, y **logins de email/Google fallidos** (2ª fase, 2026-07-24). Guarda **metadatos** (ip, user_agent, method, path, status, `trace_id` = `request.id`, details) — **NUNCA el cuerpo/headers ni la contraseña** (evita secretos/PII). Throttle por IP para no inundar. Lista en `GET /admin/security/events` (filtros type/from/to + paginación). UI: pestaña **Logs** dentro del módulo Auditoría. **Email/Google:** como pasan por Supabase Auth directamente desde el frontend (el backend no los ve), el cliente los reporta a `POST /security/auth-failed` (público, rate-limitado + throttle, solo metadatos) → mismo tipo `login_failed` con `details.method` (email/google/username). Es **advisory** (reportado por el cliente; la fuente autoritativa son los logs de Auth de Supabase). WAF (Cloudflare) → pendiente para cuando haya tenants de pago. |
...
+ coste + top referidores + submenús **Fraude** y **Configuración**), **Monitorización** (Métricas en vivo:
...
`WEBHOOKS (bandeja error/dead)` · `Groq·Voz` (whisper) · `Groq·Parser` (LLM) · `Groq·Cuota` (rate-limit) ·
...
- **Chat de flota (jefe↔conductor):** `fleet_messages` (chat directo 1:1, privado del tenant; el admin no lo ve). Reemplaza las antiguas notas `incidents kind='nota'`. El conductor ve el NOMBRE real del jefe (en chat y push): `GET /fleet/boss-name` (service_role, porque el conductor no puede leer la fila del owner por RLS). Los vehículos asignados al conductor se leen en 2 consultas (ids + vehículos), no con embed anidado (el embed bajo RLS del conductor devolvía null → "sin vehículo asignado").
- **Recuperar contraseña:** en el login, "¿Has olvidado la contraseña?" envía el enlace (`resetPasswordForEmail`, mismo redirect que Google/confirmación); al abrirlo, el evento `passwordRecovery` (main.dart) abre la pantalla de nueva contraseña.
- **Mantenimiento de vehículos:** fechas en `vehicles` (itv/taximeter_itv/insurance/transport_card + revisión por km) + `maintenance_reminders_sent` (throttle de avisos). Cron diario (maintenance-reminders.yml) avisa al jefe por push en hitos 30/15/7/1 días/día/caducado y ~1000/~200/0 km. **Periodo de renovación (mig. 083):** columnas `itv_period_months`/`taximeter_itv_period_months`/`insurance_period_months`; en la ficha el owner elige "cada 6/12/24 meses" y la app calcula la próxima caducidad (base = fecha futura actual o hoy). No cambia el cron: sigue avisando sobre la fecha.
- **Convertir empresa↔autónomo (admin):** la flag `tenants.solo` (autónomo = el owner es también su propio chófer) se puede cambiar desde el panel admin (ficha de empresa, dentro de **Modo soporte**, auditado) vía `PATCH /admin/company/:id {solo}` (companies.js, ya existía). Solo cambia la flag; no crea ni borra chóferes. Complementa la RPC `set_solo_mode` que ya tenía el propietario en Ajustes.
...
- **Pulso diario (dashboard admin):** `GET /admin/daily-metrics` — métricas agregadas de la plataforma (negocio: ingresos hoy/mes/cobros/reembolsos de Stripe; uso: carreras hoy, DAU, transcripciones; crecimiento: empresas/conductores nuevos, pruebas que acaban; producto: activación, empresas en riesgo; soporte: tickets abiertos). PROTECCIÓN DE DATOS: solo recuentos y, en €, únicamente NUESTROS ingresos, nunca el dinero de las carreras de los clientes.
...
| **Stripe** | Suscripciones per-asiento (máx. 100). **Asientos PRE-PAGADOS** = cupo de conductores (`tenants.drivers_limit`): durante la prueba se añaden sin límite; al pagar solo quedan activos los asientos pagados (bloquea los más nuevos, `enforceSeatLimit`). Ajustar asientos (`/subscription/seats`) **cobra YA y SOLO los asientos nuevos** mediante un cargo ONE-OFF desacoplado de la suscripción (invoice item de customer sin `subscription=`, factura con `pending_invoice_items_behavior:'include'` + `default_payment_method` resuelto de la suscripción/cliente/1ª tarjeta, finalize+pay al instante) + subida de cantidad con proration 'none'; así NO se pierde el cupón del pago inicial y el cargo NO queda pendiente para la renovación. El importe unitario tiene fallbacks (`unit_amount` → `unit_amount_decimal` → `plan.amount` → precio base por periodo) porque el precio puede devolver `unit_amount` null. El periodo para prorratear se lee del item. Si el cobro falla, se anula la factura y no se amplía. Reducir acredita el sobrante en la próxima factura. **Confirmación previa con el importe** (GET `/subscription/seats` da periodo+precio real). Portal, **cupón activo** (system_config `active_coupon`; los YA suscritos pueden aplicarlo con "¿Tienes un cupón?" → `/subscription/apply-coupon`, descuento en la PRÓXIMA renovación; UN solo cupón por renovación — se rechaza si ya hay descuento pendiente, ni acumular ni cambiar; el descuento aplicado sobrevive a la caducidad del cupón. Al crear un cupón nuevo, el panel RETIRA el anterior — si el nuevo está programado, lo retira el vigía al activarse, vía prev_promo_id), webhooks idempotentes (`webhook_events` + reproceso de la bandeja). **Ingresos reales cobrados** (total facturado MENOS reembolsos + descontado con cupones; "Devuelto" visible si hay refunds) se leen EN VIVO de Stripe (facturas pagadas + refunds/charges): global en el dashboard (caché 60 s) y por empresa en su ficha (protección de datos: es lo que la empresa nos paga a nosotros, NO sus finanzas internas). Ajuste de asientos EN LOTE: +/- acumula un cambio pendiente y "Aplicar" hace un solo cobro/confirmación. **Ciclo de vida (owner):** un solo plan por asiento; mensual/anual es el PERIODO de facturación (no planes distintos). Cancelar a fin de periodo (`/subscription/cancel`, `cancel_at_period_end`, reactivable). **Baja de la empresa** (`/company/close`): cancela Stripe + cierre lógico (`closeTenantAccount`: `closed_at`, retención GDPR, elimina accesos), reutilizado por el cierre del admin. **Reactivar empresa** (admin, `/admin/company/:id/reactivate`, antes de purgar): quita `closed_at`, restaura nombre, regenera código de flota, da prueba ('trialing') y crea la cuenta del nuevo owner con contraseña temporal (los conductores se re-invitan) | backend (`billing.js`, `/subscription/seats`, `/subscription/cancel`, `/company/close`, `readGlobalRevenue`/`readTenantRevenue`, `/webhooks/stripe`, `retryFailedWebhooks`) | semáforos **STRIPE** (firma) / **WEBHOOKS** (bandeja) |
| **Groq** (único proveedor IA; `OPENAI_BASE_URL=api.groq.com`) | whisper-large-v3 (voz) + llama-3.3-70b (parser LLM) | backend (`/transcribe`, `llm_parser.js`) | semáforos **Groq·Voz** / **Groq·Parser** / **Groq·Cuota** (el interno 'openai' es nombre heredado del SDK, NO es OpenAI) |
...
1. **Modularizar `server.js` (5.876 líneas) — PLAN DE EXTRACCIÓN (2026-07-24).**
   ...
   **Fase B — grupos de rutas como plugin (riesgo medio).** De cara a la tarea go-live #4
   ...
   | ✅ **Fraude (Centro de fraude)** HECHO (2026-07-28) | −112 | `fraud.js` = `registerFraudRoutes(app, { supabase, adminGuard, logAdminAction })`. 3 endpoints (visor unificado referral_fraud_alerts + fraud_alerts: lista/detalle/resolver) + 2 helpers de mapeo. Bloque LIMPIO y contiguo (L3666–3776), un solo corte. **OJO scope:** el escaneo/config anti-fraude de referidos (batch, L4047–4546) NO entra aquí → va con **Referidos**. Validado: boot + 3/3 rutas + suite integración verde. |
   | ✅ **Referidos** HECHO (2026-07-28) | −547 | `referrals.js` = `registerReferralsRoutes(app, { supabase, adminGuard, getCaller, logAdminAction, refConfig, milestonesFrom, recomputeReferrerMilestones })`. 15 endpoints (cliente + admin) + 4 helpers exclusivos INCL. el anti-fraude de referidos (createFraudAlert/runFraudChecks). El bloque resultó contiguo (L3900–4445). **Entanglements resueltos:** refConfig/milestonesFrom SE QUEDAN en server.js (los comparte rewards.js → evitar circular) y se inyectan; los helpers de cola/reversión (revertReferralForTenant, enqueue/reject, processReferralValidationQueue) SE QUEDAN porque los usa el webhook de billing.js; el cron process-referral-validations se queda. Validado: 16/16 rutas + suite integración verde (incl. webhook, que ejerce el cableado billing↔referrals). |
   | ✅ **Informes Excel/PDF + Import** HECHO (2026-07-28) | −154 | `reports_routes.js` = `registerReportsRoutes(app, { supabase, getCaller, rateLimited, withTimeout, REPORT_TIMEOUT_MS, XLSX_MIME, OPENAI_API_KEY, OPENAI_BASE_URL, LLM_PARSE_MODEL })`. `handleReport` + `/reports/excel` + `/reports/pdf` + `/import/transactions`. Importa directo fetchReportData/buildExcel/buildPdf/cache de reports.js, parseImportFile de importer.js, llmMapColumns de llm_parser.js. Bloque LIMPIO al final de buildApp (L4338–4491), sin acoplamiento cruzado. Las constantes module-level y withTimeout se inyectan. Validado con stack: 3/3 rutas + excel/pdf/importer tests verdes. |
   | ✅ **Incidencias/push** HECHO (2026-07-28) | −205 | `incidents.js` = `registerIncidentsRoutes(app, { supabase, adminGuard, getCaller, logAdminAction, notifyUsers, notifyUser, markService })`. 2 zonas: panel admin de incidencias (L1805–1887, 5 endpoints) + push cliente notify-incident/notify-fleet-message/fleet-boss-name (L2780–2899). Los helpers CORE de push (notifyUsers/notifyUser/notifyUsersRaw) SE QUEDAN en server.js (compartidos con mantenimiento/alertLimit/chat) y se inyectan; markService viene de monitoring.js. Validado: 8/8 rutas + suite verde. |
   | ✅ **Suscripción (Checkout/Portal/asientos/cupones)** HECHO (2026-07-28) | −509 | `subscription.js` = `registerSubscriptionRoutes(app, deps)` → **devuelve `{ syncScheduledCoupon }`** (lo llama el cron/vigía en server.js). 11 endpoints (asientos comprar/reducir, cancel, apply-coupon, checkout, migrate-prices, active-coupon×3, coupons, portal) + TODA la lógica de cupón (readActiveCoupon/readCouponConfigRaw/syncScheduledCoupon). Bloque contiguo (L2854–3361). Se inyectan: seatCount/setSeatQuantity/enforceSeatLimit (enforceSeatLimit lo comparte billing.js), stripe, log y las constantes module-level (STRIPE_PRICE_SEAT_*, STRIPE_SUCCESS/CANCEL_URL, **MAX_SEATS**); planForPrice se importa de billing.js. **LECCIÓN:** el test de integración cazó `MAX_SEATS is not defined` (constante module-level que node --check y el registro de rutas NO detectan; solo se ve al ejecutar el handler) → levantar el stack es clave para las rutas con constantes. Validado: 11/11 rutas + checkout/portal/webhook tests verdes. |
   | ✅ **Odómetro** HECHO (2026-07-28) | −130 | `odometer.js` = `registerOdometerRoutes(app, { supabase, adminGuard, logAdminAction })`. 5 endpoints de corrección de km (L2866–2994). Bloque LIMPIO, solo 3 deps. Primer sub-cluster del Panel admin. Validado: 5/5 rutas + suite verde. |
   | **Panel admin (núcleo)** — NO es un dominio, son ~6 sub-clusters | resto | **Auditoría 2026-07-28:** (1) ✅ **Dashboard financiero HECHO** (`financial.js` = `registerFinancialRoutes(app,{supabase,stripe,adminGuard,probeDb,log})` → **DEVUELVE `{readGlobalRevenue,readTenantRevenue}`**). overview/daily-metrics/billing + los 7 helpers financieros y sus cachés 60s (dayBounds/sumPaidInvoices/sumRefunds/readGlobalRevenue/readGlobalFees/readTenantRevenue/readMrr). Bloque contiguo L1411–1962, deps mínimas (NO usa métricas). Se registra ANTES que retos/companies para pasarles readGlobalRevenue/readTenantRevenue. −553 líneas. **3 BUGS LATENTES cazados por un scan de referencias cruzadas** (helpers usados sin inyectar, que node --check NO ve y ningún test tocaba): retos.js→readGlobalRevenue (challenges/summary), incidents.js→platformAdminIds (notify-incident), companies.js→freeDaysForTenant (ficha empresa). Los 3 arreglados inyectando. **LECCIÓN: tras cada extracción de rutas, escanear el módulo por identificadores llamados que no estén en params/imports/locales.** Validado: 4/4 rutas + suite verde. (2) ✅ **Empresas HECHO** (`companies.js` = `registerCompaniesRoutes(app,{supabase,stripe,log,adminGuard,getCaller,logAdminAction,readTenantRevenue})`; ficha empresa + search + vehicles + edit/close/delete/purge/reset-coupon/invoices/reactivate. 3 zonas L1838–1929/2043–2287/2365–2461, excluyendo billing y test-rewards que se quedan. **closeTenantAccount** estaba dentro de la zona → se movió al módulo y vive local (solo lo usan close/delete, que también movieron); readTenantRevenue inyectado. **LECCIÓN:** un helper dentro de la zona cortada SE MUEVE con ella — comprobar si tiene usos fuera (si no, vive local; el test de boot lo caza: `closeTenantAccount is not defined`)). (3) ✅ **Usuarios/admins HECHO** (`admin_users.js` = `registerAdminUsersRoutes(app,{supabase,adminGuard,logAdminAction})`; 2 zonas: admins/make-admin L1834–1866 + user CRUD/vehicles L2661–2723). (4) ✅ Odómetro HECHO (`odometer.js`). (5) ✅ **Visores auditoría HECHO** (`audit_viewers.js` = `registerAuditViewerRoutes(app,{supabase,adminGuard})`; audit/logs + security/events + client-errors, contiguo L2870–2948, solo lectura, deps supabase/adminGuard; los admin/error-reports quedan aparte con su POST cliente). (6) ✅ **Flags HECHO** (`flags.js` = `registerFlagsRoutes(app,{supabase,adminGuard,logAdminAction,flagOn,invalidateFlagCache})`; 2 endpoints /admin/flags + KNOWN_FLAGS local; L2802–2837). ✅ **Metrics HECHO (2026-07-28)** — `metrics.js` = `registerMetricsRoutes(app,{supabase,adminGuard,supabaseMetrics})`. Endpoint /admin/metrics + groqUsage/groqModelPct/inputActivity (metrics-only, 2 zonas). `supabaseMetrics` (+parsePromMetrics/cpuPctFromCounters) se INYECTA: se queda en server.js porque lo comparte el cron de semáforos; `markGroqRateLimit` también se queda (lo usa parseSmart del cliente). Cross-ref scan limpio. Validado: rutas + suite verde. **→ Panel admin 100% repartido.** + ~7 crons dispersos (misc infra). Hacer por sub-cluster, del más limpio (odómetro✅→visores→usuarios→empresas) al financiero (último, cuidadoso). |
