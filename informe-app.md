...
| `reports.js` | Generación de Excel (`exceljs`) y PDF (`pdfmake`). **Excel**: filas ordenadas por cliente (`orderedDetail`) — ingresos agrupados por cliente (Particular primero, resto alfabético), **fila en blanco entre grupos**, ordenados por fecha/hora; los **gastos al final**, por fecha. Importes **numéricos con formato € (EUR)** (`MONEY_FMT`; multi-moneda = futuro). Resumen en negrita con los **gastos encima** de los ingresos: gastos como **categoría · método de pago en la misma línea** → TOTAL Gastos; ingresos por método → TOTAL Ingresos; BALANCE. Un gasto **"Otros"** con texto libre (p. ej. ITV) usa ese texto (`expenseCatKey`). El **PDF** conserva el desglose `summaryBlock` (orden anterior; no reordena por cliente). |
