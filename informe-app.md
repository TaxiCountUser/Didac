...
| `reports.js` | Generación de Excel (`exceljs`) y PDF (`pdfmake`). El Excel cierra cada hoja con un **resumen ampliado en negrita**: ingresos por método de pago → TOTAL Ingresos; gastos por método de pago **y** por categoría → TOTAL Gastos; BALANCE (Ingresos − Gastos). El **PDF** lleva el mismo desglose (`summaryBlock`): resumen global + uno por conductor. Un gasto de categoría **"Otros"** con texto libre (p. ej. ITV) se muestra por ese texto en el concepto y como su propia línea en "Gastos por categoría" (`expenseCatKey`). |
