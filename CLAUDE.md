...
**Fase B EN CURS**: ✅ `retos.js` (7) · ✅ `fraud.js` (3) · ✅ `referrals.js` (15 + anti-frau) · ✅ `reports_routes.js` (Excel/PDF+Import) · ✅ `incidents.js` (incidents+push) · ✅ `subscription.js` (checkout/portal/seients/cupó) · ✅ `odometer.js` (correcció km, 5 endpoints) → pendent **Panel admin** (NO és 1 domini: 6 sub-clústers — financer/empreses/usuaris/odòmetre✅/visors-auditoria/flags; fer per sub-clúster, el financer l'últim per sensibilitat diners/RGPD)…
...
va caçar `MAX_SEATS is not defined` que node --check NO veu). server.js: ~5.9k → ~3.55k línies. Agrupar rutes admin de cara a
