...
- **Agenda (OCULTA i de pagament, Fase 1, 2026-08-27):** opció que **l'admin activa empresa a empresa** (`tenants.agenda_enabled`, mig. 084) — invisible si està OFF. `data_service.isAgendaEnabled()` és **aïllat i resilient** (try/catch → false); **NO** va a `fetchMyTenantState` a posta (això ho usa `auth_gate`, crític). Pantalles `agenda_input_screen.dart` (form: preu, dia/hora, client, recollida, destí, telèfon/nom, nota) + `agenda_screen.dart` (taula `agenda_events`, compartida per empresa via RLS). Accés gateat als **3 homes** (driver_home BigButton · owner_dashboard icona toolbar · autònom via driver_home embedded) + **3r segment "Agenda"** a `transaction_input`. **Veu**: "apunta en la agenda / a l'agenda / add to agenda" a `add_record._onParsed`. Toggle admin a `admin_company_detail`. Claus `ag_*`/`dh_agenda`/`ti_agenda`/`admin_agenda*`. Google Calendar = **Fase 2** (pendent). Detall a la memòria `agenda-feature`.
