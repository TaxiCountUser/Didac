...
va caçar `MAX_SEATS is not defined` que node --check NO veu; i `closeTenantAccount is not defined` quan un helper DINS la zona tallada es mou amb ella). server.js: ~5.9k → ~2.94k línies. Agrupar rutes admin de cara a
